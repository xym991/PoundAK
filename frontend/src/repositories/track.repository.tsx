// A helper function that clamps the number and returns a Percentage.
export function percent(n: number, m?: number) {
  if (n < 0) n = 0;
  // if (n > 100) n = 100;
  return `${n}%`;
}

export function addPercentages(m: string, n: string): string {
  return `${(Number(m?.replace("%", "")) * Number(n?.replace("%", ""))) / 100}%`;
}
...
export type performanceTrackTask = {
  locked?: boolean;
  condition?: string;
  tag?: string;
  value: string;
...
export type performanceTrackItem = {
  value: string;

  tasks: performanceTrackTask[];
};
export const trackData: performanceTrackItem[] = [
  //fitness
  {
    value: percent(0),
